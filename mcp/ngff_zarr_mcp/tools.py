            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if options.dims:
                    if len(options.dims) != len(ngff_image.dims):
                        raise ValueError(f"Provided dims count ({len(options.dims)}) doesn't match image dims ({len(ngff_image.dims)})")
                ...
                if scale_factors is None:
                    # Single scale output
                    multiscales_obj = type('Multiscales', (), {
                        'images': [ngff_image]
                    })()
                else:
                    # Generate multiscales
                    method = Methods(options.method) if options.method else Methods.ITKWASM_GAUSSIAN
...
async def validate_ome_zarr(store_path: str) -> ValidationResult:
    ...
    try:
        ...
        try:
            if hasattr(zarr.storage, 'DirectoryStore'):
                store = zarr.storage.DirectoryStore(store_path) if not is_url(store_path) else store_path
            else:
                store = zarr.storage.LocalStore(store_path) if not is_url(store_path) else store_path
...
async def optimize_zarr_store(options: OptimizationOptions) -> ConversionResult:
    ...
    try:
        ...
        optimized_multiscales = type('Multiscales', (), {
