...
try:
    ...
    def is_unit_supported(unit):
        common_units = ['meter', 'millimeter', 'micrometer', 'nanometer', 'second', 'millisecond']
...
def analyze_zarr_store(store_path: str) -> StoreInfo:
    ...
    try:
        ...
        return StoreInfo(
            ...
            chunks=list(first_image.data.chunks[0]) if hasattr(first_image.data, 'chunks') else [],
            compression=compression,
            scale_info=dict(first_image.scale) if first_image.scale else {},
            translation_info=dict(first_image.translation) if first_image.translation else {}
        )
...
def get_supported_formats() -> SupportedFormats:
    ...
    input_formats = {
        ...
        "itkwasm": [
            ".bmp", ".dcm", ".gipl", ".gipl.gz", ".hdf5", ".jpg", ".jpeg",
            ".iwi", ".iwi.cbor", ".iwi.cbor.zst", ".lsm", ".mnc", ".mnc.gz",
            ".mnc2", ".mgh", ".mhz", ".mha", ".mhd", ".mrc", ".nia", ".nii",
            ".nii.gz", ".hdr", ".nrrd", ".nhdr", ".png", ".pic", ".vtk",
            ".aim", ".isq", ".fdf"
        ],
        "itk": [
            ".bmp", ".dcm", ".gipl", ".gipl.gz", ".hdf5", ".jpg", ".jpeg",
    ...
    backends = [
        "ngff_zarr", "zarr", "itkwasm", "itk", "tifffile", "imageio"
    ]
...
def setup_dask_config(memory_target: Optional[str] = None, 
                     use_local_cluster: bool = False,
                     cache_dir: Optional[str] = None) -> Optional[Any]:
...
def validate_conversion_options(options: Dict[str, Any]) -> List[str]:
    ...
    if 'dims' in options and options['dims']:
        ...
        if invalid_dims:
            errors.append(f"Invalid dimensions: {invalid_dims}. Must be from {valid_dims}")
    ...
    if 'method' in options:
        ...
        if options['method'] not in available_methods:
            errors.append(f"Invalid method '{options['method']}'. Available: {available_methods}")
...
async def prepare_input_files(input_paths: List[str], temp_dir: Optional[str] = None) -> List[str]:
