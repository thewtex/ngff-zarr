...
class ConversionOptions(BaseModel):
    ...
    ome_zarr_version: Literal["0.4", "0.5"] = Field("0.4", description="OME-Zarr version")
    
    # Metadata options
    dims: Optional[List[str]] = Field(None, description="Ordered NGFF dimensions from {t,z,y,x,c}")
    scale: Optional[Dict[str, float]] = Field(None, description="Scale/spacing for each dimension")
    translation: Optional[Dict[str, float]] = Field(None, description="Translation/origin for each dimension")
    units: Optional[Dict[str, str]] = Field(None, description="Units for each dimension")
    name: Optional[str] = Field(None, description="Image name")
    
    # Processing options
    chunks: Optional[Union[int, List[int]]] = Field(None, description="Dask array chunking specification")
    chunks_per_shard: Optional[Union[int, List[int]]] = Field(None, description="Chunks per shard for sharding")
    method: str = Field("itkwasm_gaussian", description="Downsampling method")
    scale_factors: Optional[Union[int, List[Union[int, Dict[str, int]]]]] = Field(None, description="Scale factors for multiscale")
    
    # Storage options
    compression_codec: Optional[str] = Field(None, description="Compression codec (gzip, lz4, zstd, blosc)")
    ...
    use_local_cluster: bool = Field(False, description="Use Dask LocalCluster for large datasets")
...
class SupportedFormats(BaseModel):
    ...
    input_formats: Dict[str, List[str]] = Field(..., description="Supported input formats by backend")
...
class OptimizationOptions(BaseModel):
    ...
    chunks_per_shard: Optional[Union[int, List[int]]] = Field(None, description="New sharding configuration")
