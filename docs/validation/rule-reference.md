...
The nineteen RFC 8 node/collection rules (16–34) are normative from OME-Zarr
...
| 21| `node-nodes-xor-path` | RFC-8 collections and multiscales | A collection or multiscale node carries exactly one of `nodes` and `path`; `singlescale` (optional `path`, no `nodes`), unknown and extension node types are exempt. Same gating as rule 16. | RFC-8: either `nodes` or `path` MUST be present, but not both. | `ome.nodes[1]` |
| 22| `path-type-known` | RFC-8 paths | A path `type` is `zarr`, `json`, or a prefixed extension identifier (`prefix:name`), which passes as opaque. Applies to every node's own `path` and to the `path` of every reference site. Same gating as rule 16. | RFC-8: unprefixed path types are reserved for the core specification, which defines `zarr` and `json`. | `ome.nodes[1].path.type` |
| 23| `coordinate-system-id-required` | RFC-8 coordinate systems | Every coordinate system declared under a node's `attributes.coordinateSystems`, or under its `attributes.scene.coordinateSystems`, carries an `id` (the `name` becomes an optional label). Same gating as rule 16. | RFC-8: coordinate systems are referenced by id, not by name. | `ome.nodes[0].attributes.coordinateSystems[1]` |
| 24| `reference-id-required` | RFC-8 references | Every top-level transformation `input`/`output` under `attributes.coordinateTransformations` or `attributes.scene.coordinateTransformations`, and every labels `source` entry, is an object carrying an `id`; wrapped transforms inside a sequence are exempt. Same gating as rule 16. | RFC-8: transformation inputs and outputs, and label sources, are References. | `ome.nodes[0].attributes.coordinateTransformations[0].input` |
| 25| `reference-path-required` | RFC-8 references | A reference whose `id` is not declared in this document carries a `path` locating its document; a transformation reference resolves against node and coordinate-system ids, a labels `source` reference against multiscale node ids only. What the path points at is not resolved. Same gating as rule 16. | RFC-8: for external references, the path field MUST be present. | `ome.nodes[0].attributes.coordinateTransformations[0].output` |
...
| 32| `node-version-consistent` | RFC-8 nodes | A non-root node's `version`, when declared, equals the root's; a no-op when the root declares none. Same gating as rule 16. | RFC-8: non-root nodes MUST NOT have a different version value than the root. | `ome.nodes[2].version` |
| 33| `singlescale-transform-shape` | RFC-8 singlescales | Declared singlescale transformations are a single `scale`, or a `sequence` of a `scale` followed by a `translation`, and the `input` references the node's own id; inlined entries without attributes (the distributed layout) are exempt. Same gating as rule 16. | RFC-8: singlescale coordinateTransformations contain a single scale, or scale then translation, with input referencing the singlescale itself. | `ome.nodes[0].attributes.coordinateTransformations` |
| 34| `multiscale-output-consistent` | RFC-8 multiscales | Every inline singlescale transformation output references one and the same coordinate system, declared under the multiscale's `coordinateSystems`. Same gating as rule 16. | RFC-8: the output fields of all transformations reference the same coordinate system via the id field. | `ome.nodes[1].attributes.coordinateTransformations[0].output` |
...
- **`validate_collection` / `validateCollection`** evaluates rules **16–34**
...
The HCS rules (14 and 15) and the RFC-8 rules (16–34) are deliberately absent
...
- **RFC 8 nodes / collections** (normative from 0.9.dev3) —
  ...
  `node-version-consistent`, `singlescale-transform-shape`,
