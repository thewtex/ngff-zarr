...
| 29| `plate-columns-rows-required` | RFC-8 HCS | A declared `plate` attribute carries non-empty `columns` and `rows` arrays; the entry ids join the id walks of rules 19 and 20. Same gating as rule 16. | RFC-8: plate `columns` and `rows` are required. | `ome.attributes.plate` |
| 30| `well-reference-resolves` | RFC-8 HCS | A `well` attribute's `column` and `row` are id references naming entries the nearest enclosing plate declares. Same gating as rule 16. | RFC-8: a well references one of the columns/rows listed on the enclosing plate. | `ome.nodes[0].attributes.well.column` |
| 31| `acquisition-reference-resolves` | RFC-8 HCS | An `acquisition` attribute is an id reference naming one of the acquisitions the nearest enclosing plate declares. Same gating as rule 16. | RFC-8: the acquisition attribute references one of the acquisitions. | `ome.nodes[0].nodes[1].attributes.acquisition` |
