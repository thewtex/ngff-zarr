...
def _compute_next_translation(previous_image, dim_factors):    
    ...
    input_index = {dim: 0.5 * (dim_factors[dim] - 1)
                              for dim in previous_image.dims if dim in dim_factors}
    # Translate input index coordinate to offset in physical space
    # NOTE: This method fails to account for direction matrix
    return {dim: input_index[dim] * input_scale[dim] + input_translation[dim]
                      for dim in previous_image.dims if dim in dim_factors}
...
def _get_truncate(previous_image, sigma_values, truncate_start=4.0) -> float:
    ...
    while any([border_len > image_len for border_len, image_len in zip(border, previous_image.data.shape)]):
...
def _downsample_dask_image(ngff_image: _NgffImage, default_chunks, out_chunks, scale_factors, label=False):
    ...
    for scale_factor in scale_factors:
        ...
        output_shape = [int(image_len / shrink_factor)
            for image_len, shrink_factor in zip(previous_image.data.shape, shrink_factors)]
